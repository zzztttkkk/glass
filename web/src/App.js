...
function makeResponsiveTheme(theme) {
    const breakpoints = {small: 320, medium: 600, large: 1280};
    const ResponsiveTheme = Object.keys(breakpoints).reduce(
        (acc, key) => {
            switch (key) {
                case "small": {
                    acc.mediaQuery[key] = `@media screen and (max-width: 600px)`;
                    return acc;
                }
                case "medium": {
                    acc.mediaQuery[key] = `@media screen and (min-width: 601px) and (max-width: 1280)`;
                    return acc
                }
                default: {
                    acc.mediaQuery[key] = `@media screen and (min-width: 1281px)`;
                    return acc
                }
            }
        },
...
function Inner() {
    ...
    utils.glass.setTheme = (v) => setTheme(makeResponsiveTheme(v));
